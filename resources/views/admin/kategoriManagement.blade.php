                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="d-flex">
                                   ...
                                   <a class="btn btn-warning me-2" href="{{ url('/admin/update-Kategori/' . $item->id) . '/edit' }}"><i class="bx bx-pencil"></i>Edit</a>
                                            <form action="{{ url('/admin/kategori-Management/' . $item->id) }}" method="POST">
                                                @method('delete')
                                                @csrf
...
<!-- Modal for Update -->
{{-- <form action="{{ route('admin.kategori-Management.update') }}" method="POST">
    @csrf
    <div class="modal fade create-kategori" tabindex="-1" role="dialog" aria-labelledby="modal_kategori"
        aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-centered">
...
{{-- <script>
    ...
    function openEditModal(id, kategori) {
        ...
        var editInput = document.getElementById('edit_kategori');
        var kategoriIdInput = document.getElementById('kategori_id');

        editForm.action = "{{ route('admin.kategori-Management', ':id') }}".replace(':id', id);
        editInput.value = kategori;
